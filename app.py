...
with tab1:
    ...
    # 3. Номер телефона в общем стиле
    st.write("---")
    phone_number = pi.render_phone_keyboard()

    # 4. НЕОБЯЗАТЕЛЬНЫЕ ДАННЫЕ
    st.write("---")
    ...
    st.write("---")
    birth_date_str = dp.render_date_picker(
        label="Дата рождения", key_prefix="main_birth"
    )

    # КНОПКА СОХРАНЕНИЯ
    st.write("---")
    if st.button(
        "🔥 СОХРАНИТЬ ЗАПИСЬ", type="primary", use_container_width=True
    ):
        # Проверяем строго обязательные поля (теперь включая Отчество)
        if not last_name.strip():
            ...
            st.error(
                "Введите ПОЛНЫЙ номер телефона (нажмите на поле телефона выше)!"
            )
        else:
            # Собираем полное ФИО, так как все три поля гарантированно заполнены
            ...
            final_number = (
                p_number.strip()
                if p_number.strip()
                else f"б/н-{int(datetime.timestamp(datetime.now()))}"
            )
            ...
            new_record = {
                ...
                "passport_date": (
                    p_date.strip() if p_date.strip() else "Не указана"
                ),
                "passport_code": (
                    p_code.strip() if p_code.strip() else "Не указан"
                ),
                "phone": phone_number,
                "district": district,
                ...
                "feed_type": (
                    feed_type.strip() if feed_type.strip() else "Не указан"
                ),
                "photo_path": "No photo",
