...
with tab1:
    ...
    st.write("---")
    # Получаем результат работы умного телефонного модуля
    ...
    if st.button("🔥 СОХРАНИТЬ ЗАПИСЬ", type="primary", use_container_width=True):
        ...
        # Умная проверка телефона на ошибки длины
        elif phone_number == "НЕ_ЗАПОЛНЕН":
            st.error("Укажите номер телефона получателя!")
        elif phone_number == "ОШИБКА_ДЛИНЫ":
            st.error("Ошибка! В стандартном номере должно быть ровно 10 цифр после +7. Либо переключитесь на 'Иной формат номера'.")
        else:
            ...
            new_record = {
                ...
                "phone": phone_number, # Сюда запишется либо красивый мобильный, либо ручной иной формат
                "district": district,
            ...
            if success:
                ...
                # Полный сброс состояний телефона для чистого следующего ввода
