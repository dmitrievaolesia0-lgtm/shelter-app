...
def part_2_filter_data(df, sort_options, selected_sort, selected_districts):
    """Часть 2: Поиск, фильтрация и компактный вывод служебной информации"""
    ...
    if total_records > limit_per_page:
        ...
        with page_col1:
            if st.button("⬅️ Назад", disabled=st.session_state.current_page_num == 1, use_container_width=True):
                ...
                st.rerun()
        with page_col2:
            st.markdown(f"<p style='text-align: center; margin-top: 5px; color: #666; font-size: 13px;'>Страница {st.session_state.current_page_num} из {total_pages}</p>", unsafe_allow_html=True)
        with page_col3:
